 ...
 */
abstract contract BaseOmniValidator is IOmniValidator {
    event ValidatorInited(address indexed wallet);
    event ValidatorCleared(address indexed wallet);
    ...
    modifier onlyEnabledValidator() {
        require(
            VersaOmniWallet(payable(msg.sender)).isValidatorEnabled(address(this)),
            "OmniValidator: this validator is not enabled"
        );
        _;
    }
    ...
    function initWalletConfig(bytes memory data) external onlyEnabledValidator {
        if (!_isWalletInited(msg.sender)) {
            _init(data);
            emit ValidatorInited(msg.sender);
        }
    }
    ...
    function clearWalletConfig() external onlyEnabledValidator {
        if (_isWalletInited(msg.sender)) {
            _clear();
            emit ValidatorCleared(msg.sender);
        }
    }
    ...
    function getSyncInitData(address wallet) external view virtual returns (bytes memory validatorInitData) {}

    /**
     * @dev Inherits from ERC165.
     */
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IOmniValidator).interfaceId;
    }
